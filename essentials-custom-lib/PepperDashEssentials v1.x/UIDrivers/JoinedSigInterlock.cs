...
namespace PepperDash.Essentials
{
    public class JoinedSigInterlock
    {
        ...
        public void ShowInterlocked(uint join)
        {
            ...
            //Debug.LogMessage(2, "Trilist {0:X2}, interlock swapping {1} for {2}", TriList.ID, CurrentJoin, join);
        ...
        public void HideAndClear()
        {
            var prevJoin = CurrentJoin;
            var wasShown = IsShown;
            //Debug.LogMessage(2, "Trilist {0:X2}, interlock hiding {1}", TriList.ID, CurrentJoin);
            Hide();
            CurrentJoin = 0;
        ...
        public void Show()
        {
            ...
            //Debug.LogMessage(2, "Trilist {0:X2}, interlock showing {1}", TriList.ID, CurrentJoin);
            if (CurrentJoin > 0)
            {
                TriList.BooleanInput[CurrentJoin].BoolValue = true;
                IsShown = true;
