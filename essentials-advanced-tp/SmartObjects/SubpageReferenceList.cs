    ...
    public class SubpageReferenceList
    {
            ...
            uint boolIncrement, uint ushortIncrement, uint stringIncrement)
        {
            ...
            if (triList.SmartObjects.TryGetValue(smartObjectId, out obj))
            {
                ...
                Debug.LogMessage(LogEventLevel.Information, "SRL {0} contains max {1} items", SRL.ID, MaxDefinedItems);
        ...
        public BoolInputSig BoolInputSig(string sigName)
        {
            if (!SRL.BooleanInput.Contains(sigName)) return null;
            return SRL.BooleanInput.FirstOrDefault(s => s.Name.Equals(sigName));
        }

        public void SetInputVisible(uint index, bool state)
        {
            var sig = BoolInputSig(String.Format("Item {0} Visible", index));
            if (sig != null)
                sig.BoolValue = state;
        }
        public void SetInputEnable(uint index, bool state)
        {
            var sig = BoolInputSig(String.Format("Item {0} Enable", index));
            if (sig == null)
                sig = BoolInputSig(String.Format("Item {0} Enabled", index));
            if (sig != null)
                sig.BoolValue = state;
        }
        public bool GetInputVisible(uint index)
        {
            var sig = BoolInputSig(String.Format("Item {0} Visible", index));
            if (sig == null) return false;
            return sig.BoolValue;
        }
        public bool GetInputEnable(uint index)
        {
            var sig = BoolInputSig(String.Format("Item {0} Enable", index));
            if (sig == null)
                sig = BoolInputSig(String.Format("Item {0} Enabled", index));
