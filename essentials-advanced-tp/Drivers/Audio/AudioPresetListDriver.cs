...
namespace essentials_advanced_tp.Drivers
{
    public class AudioPresetListDriver : PanelDriverBase, IAdvancedRoomSetup
    {
        ...
        public void Setup(IAdvancedRoom room)
        {
            ...
            {
                Debug.LogMessage(LogLevel, "{0} Setup CurrentRoom.Audio.Presets {1}", ClassName, CurrentRoom_.Audio.Presets == null ? " = null" : "exists");
                foreach (var preset in CurrentRoom_.Audio.Presets)
                {
                    Debug.LogMessage(LogLevel, "{0} Setup Presets {1}, {2}", ClassName, preset.Key, preset.Value.Name);
                    sol.SetItemVisible(i, true);
                    sol.SetItemMainText(i, preset.Value.Name);
                    var buttonActionAssigned = false;
                    Debug.LogMessage(LogLevel, "{0} Setup preset.Value.Level {1}", ClassName, preset.Value.Level == null ? " = null" : "exists");
                    if (preset.Value.Level != null)
                    {
                        Debug.LogMessage(LogLevel, "{0} Setup preset.Value.Level.CurrentDevice {1}", ClassName, preset.Value.Level.CurrentDevice == null ? " = null" : preset.Value.Level.CurrentDevice.Key);
                        var fbDev = preset.Value.Level.CurrentDevice as IBasicVolumeWithFeedback;
                        Debug.LogMessage(LogLevel, "{0} Setup fbDev {1}", ClassName, fbDev == null ? " = null" : "exists");
                        if (fbDev != null)
                        {
                            sol.SetItemButtonAction(i, (o) => {
                                Debug.LogMessage(LogLevel, "{0} RecallPreset (level) {1} SmartObject pressed: {2}", ClassName, i.ToString(), preset.Value.Level.CurrentDevice.Key);
                                fbDev.SetVolume(1);
                            });
                            buttonActionAssigned = true;

                            var ItemFeedback = sol.GetBoolFeedbackSig(i);
                            Debug.LogMessage(LogLevel, "{0} Setup ItemFeedback {1}", ClassName, ItemFeedback == null ? " = null" : "exists");
                            if (ItemFeedback != null)
                            {
                                EventHandler<FeedbackEventArgs> Feedback_OutputChange = (object o, FeedbackEventArgs e) =>
                                {
                                    Debug.LogMessage(LogLevel, "{0} VolumeLevelFeedback.OutputChange[{1}]: {2}", ClassName, i, e.IntValue);
                                    ItemFeedback.BoolValue = e.IntValue > 0;
                                };
                                fbDev.VolumeLevelFeedback.OutputChange -= Feedback_OutputChange;
                                fbDev.VolumeLevelFeedback.OutputChange += Feedback_OutputChange;
                                Debug.LogMessage(LogLevel, "{0} Setup VolumeLevelFeedback", ClassName);
                            }
                        }
                    }
                    if (!buttonActionAssigned)
                        sol.SetItemButtonAction(i, (o) => {
                            Debug.LogMessage(LogLevel, "{0} RecallPreset {1} SmartObject pressed: {2}", ClassName, i.ToString(), preset.Value.Level.CurrentDevice.Key);
            ...
            {
                Debug.LogMessage(LogLevel, "{0} Setup SetItemInvisible: {1}", ClassName, i);
