...
{
    public class PowerDriver : PanelDriverBase, IAdvancedRoomSetup
    {
        ...
        public void Setup(IAdvancedRoom room)
        {
            ...
            Debug.LogMessage(LogLevel, "{0} Setup, connect, room {1}", ClassName, CurrentRoom == null ? "== null" : CurrentRoom.Key);
            ...
            if (CurrentRoom != null)// Connect current room
            {
                ...
                {
                    Debug.LogMessage(LogLevel, "{0}  CurrentRoom.OnFeedback.OutputChange: {1}", ClassName, a.BoolValue);
                ...
                Debug.LogMessage(LogLevel, "{0} Setup, subscribing to ShutdownPromptTimer", ClassName);
                // Shutdown timer
                CurrentRoom.ShutdownPromptTimer.HasStarted += ShutdownPromptTimer_HasStarted;
                CurrentRoom.ShutdownPromptTimer.HasFinished += ShutdownPromptTimer_HasFinished;
                CurrentRoom.ShutdownPromptTimer.WasCancelled += ShutdownPromptTimer_WasCancelled;

                Debug.LogMessage(LogLevel, "{0} Setup, subscribing to OnFeedback", ClassName);
                ...
                Debug.LogMessage(LogLevel, "{0} Setup, subscribing to Power.PowerChange", ClassName);
                var roomPower_ = CurrentRoom as IHasPowerFunction;
                Debug.LogMessage(LogLevel, "{0} CurrentRoom as IHasPowerFunction {1}", ClassName, roomPower_ == null ? " == null" : "exists");
                if (roomPower_ != null)
                {
                    Debug.LogMessage(LogLevel, "{0} Registering Power_PowerChange", ClassName);
                    roomPower_.Power.PowerChange += Power_PowerChange;
                }
            }
        ...
        /// </summary>
        public void RefreshCurrentRoom(IAdvancedRoom room)
        {
            Debug.LogMessage(LogLevel, "{0} RefreshCurrentRoom", ClassName);
            Setup(room);
        }


        //void SetCurrentRoom(IAdvancedRoom room)
        //{
        //    Debug.LogMessage(LogLevel, "{0} SetCurrentRoom", ClassName);
        ...
        {
            Debug.LogMessage(LogLevel, "{0} room_ConfigChanged", ClassName);
            RefreshCurrentRoom(CurrentRoom);
        }

        public override void Hide()
        {
            Debug.LogMessage(LogLevel, "{0} Hide", ClassName);
        ...
        {
            Debug.LogMessage(LogLevel, "{0} CurrentRoom_SyncOnFeedback", ClassName);
            var value = CurrentRoom.OnFeedback.BoolValue;
            //Debug.LogMessage(LogLevel, CurrentRoom, "UI: Is on event={0}", value);
        ...
        {
            Debug.LogMessage(LogLevel, "{0} CurrentRoom_OnFeedback_OutputChange", ClassName);
            CurrentRoom_SyncOnFeedback();
        }
        void CurrentRoom_IsWarmingFeedback_OutputChange(object sender, EventArgs e)
        {
            Debug.LogMessage(LogLevel, "{0} IsWarmingFeedback: {1}-{2}", ClassName, CurrentRoom.IsWarmingUpFeedback.BoolValue, CurrentRoom.IsWarmingUpFeedback.IntValue);
        ...
        {
            Debug.LogMessage(LogLevel, "{0} IsCoolingFeedback: {1}-{2}", ClassName, CurrentRoom.IsCoolingDownFeedback.BoolValue, CurrentRoom.IsCoolingDownFeedback.IntValue);
        ...
        {
            Debug.LogMessage(LogLevel, "{0} PowerOffPress", ClassName);
            if (!CurrentRoom.OnFeedback.BoolValue
                || CurrentRoom.ShutdownPromptTimer.IsRunningFeedback.BoolValue)
            {
                Debug.LogMessage(LogLevel, "{0} PowerOffPress cancelled, system is off or powering", ClassName);
                return;
            }
        ...
        {
            Debug.LogMessage(LogLevel, "{0} PowerOnPress", ClassName);
        ...
        {
            Debug.LogMessage(LogLevel, "{0} PowerButtonPressed, OnFeedback: {1}", ClassName, CurrentRoom.OnFeedback.BoolValue);
        ...
        //      {
        //          Debug.LogMessage(LogLevel, "{0} CancelPowerOffTimer", ClassName);
        //          Dispose();
        //      }
        ...
        {
            Debug.LogMessage(LogLevel, "{0} Power_PowerChange, current: {1}, {2} seconds remaining", ClassName, args.Current.ToString(), args.SecondsRemaining.ToString());
            try
            {
                if (ribbonDriver != null)
                {
                    Debug.LogMessage(LogLevel, "{0} Power_PowerChange ribbonDriver != null", ClassName);
                    ...
                    else
                    {
                        ...
                        Debug.LogMessage(LogLevel, "{0} Power_PowerChange msg_: {1}", ClassName, msg_);
                        ribbonDriver.ShowNotificationRibbon(msg_, (int)(args.SecondsRemaining * 1000));
                    }
                }
                Debug.LogMessage(LogLevel, "{0} Power_PowerChange ribbonDriver done", ClassName);
            }
            catch (Exception e)
            {
                Debug.LogMessage(LogLevel, "{0} Power_PowerChange ERROR: {1}", ClassName, e.Message);
            }
            Debug.LogMessage(LogLevel, "{0} Power_PowerChange done", ClassName);
        }
        ...
        {
            Debug.LogMessage(LogLevel, "{0} ShutdownPromptTimer_HasStarted: {1}", ClassName, CurrentRoom.ShutdownType);
            ...
            if (CurrentRoom.ShutdownType == eShutdownType.Manual || CurrentRoom.ShutdownType == eShutdownType.Vacancy)
            {
                ...
                offHandler = (o, a) =>
                {
                    Debug.LogMessage(LogLevel, "{0} offHandler: {1}", ClassName, onFb.BoolValue);
                    if (!onFb.BoolValue)
                    {
                ...
                PowerDownModal.PresentModalDialog(2, "System Power", "Power", message, "Cancel", "Confirm", true, true,
                    but =>
                    {
                        ...
                        {
                            Debug.LogMessage(LogLevel, "{0} PresentModalDialog Cancel button: {1}", ClassName, but);
                            timer.Cancel(); // this will invoke ShutdownPromptTimer_WasCancelled
                        }
                        else
                        {
                            Debug.LogMessage(LogLevel, "{0} PresentModalDialog Finish button: {1}", ClassName, but);
        ...
        {
            Debug.LogMessage(LogLevel, "{0} ShutdownPromptTimer_HasFinished", ClassName);
            //EndMeetingButtonSig.BoolValue = false;
            CurrentRoom.ShutdownPromptTimer.TimeRemainingFeedback.OutputChange -= ShutdownPromptTimer_TimeRemainingFeedback_OutputChange;
            CurrentRoom.ShutdownPromptTimer.PercentFeedback.OutputChange -= ShutdownPromptTimer_PercentFeedback_OutputChange;
        ...
        {
            Debug.LogMessage(LogLevel, "{0} ShutdownPromptTimer_WasCancelled", ClassName);
        ...
        {
            Debug.LogMessage(LogLevel, "{0} ShutdownPromptTimer_TimeRemaining: {1}", ClassName, (sender as StringFeedback).StringValue);
        ...
        {
            Debug.LogMessage(LogLevel, "{0} ShutdownPromptTimer_PercentFeedback: {1}%", ClassName, (sender as IntFeedback).UShortValue);
        ...
        {
            Debug.LogMessage(LogLevel, "{0} PopupInterlock_StatusChanged", ClassName);
