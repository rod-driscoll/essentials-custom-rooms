...
namespace essentials_advanced_room.Functions.Audio
{
    ...
    public class RoomAudio: ILogClassDetails
    {
        ...
        public RoomAudio(Config config)
        {
            ...
            {
                Debug.LogMessage(LogLevel, "{0} DefaultAudioKey: {1}", ClassName, config.DefaultAudioKey);
            ...
            {
                Debug.LogMessage(LogLevel, "{0} DefaultMicKey: {1}", ClassName, config.DefaultMicKey);
            ...
            if (config.Faders != null) // list of faders for tech page
            {
                ...
                {
                    Debug.LogMessage(LogLevel, "{0} fader: {1}, name: {2}, key: {3}", ClassName, item_.Key, item_.Value.Label, item_.Value.DeviceKey);
                    device_ = DeviceManager.GetDeviceForKey(item_.Value.DeviceKey);
                    if (device_ != null)
                    {
                        var level_ = new RoomVolumeLevel(device_, item_.Value.Label);
                        //Debug.LogMessage(LogLevel, "{0} fader: {1}, name: {2}{3}", ClassName, item_.Key, item_.Value.Label, level_ == null ? "== null" : "exists");
                        Levels.Add(item_.Key, level_);
                        //Debug.LogMessage(LogLevel, "{0} Added", ClassName);
                        if (item_.Value.Level > 0)
                            level_.DefaultVolume = (ushort)item_.Value.Level;
                        //Debug.LogMessage(LogLevel, "{0} DefaultVolume", ClassName);
                        //Debug.LogMessage(LogLevel, "{0} DefaultVolume {1}", ClassName, level_.DefaultVolume);
                    }
                }
            }
            Debug.LogMessage(LogLevel, "{0} faders loaded", ClassName);
            Debug.LogMessage(LogLevel, "{0} Presets {1}", ClassName, config.AudioPresets == null ? "== null" : "exist");
            ...
            if (config.AudioPresets != null)
            {
                ...
                {
                    Debug.LogMessage(LogLevel, "{0} preset: {1}, name: {2}, key: {3}, function: {4}", ClassName, item_.Key, item_.Value.Label, item_.Value.DeviceKey, item_.Value.Function);
                    // preset: system-on, name: System on , key: preset-1
                    var preset_ = new RoomAudioPreset(item_.Value);
                    //Debug.LogMessage(LogLevel, "{0} preset {1} ", ClassName, preset_==null ? "== null":"exists");
                    Presets.Add(item_.Key, preset_);
                }
            }
            Debug.LogMessage(LogLevel, "{0} presets loaded", ClassName);
        ...
        public void CustomActivate()
        {
            ...
            {
                Debug.LogMessage(LogLevel, "{0} CustomActivate loading PropertiesConfig.Volumes.Master.Level", ClassName);
        ...
        {
            Debug.LogMessage(LogEventLevel.Debug, "{0} Restoring default levels", ClassName);
            Debug.LogMessage(LogEventLevel.Debug, "{0} config.AudioPresets {1}", ClassName, Presets == null? "== null": Presets.Count.ToString());
        ...
        { 
            Debug.LogMessage(LogEventLevel.Debug, "{0} PresetRecall", ClassName);
            if (Presets?.Count > 0)
            {
                ...
                {
                    Debug.LogMessage(LogEventLevel.Debug, "{0} kvp: {1}", ClassName, kvp_.Key);
