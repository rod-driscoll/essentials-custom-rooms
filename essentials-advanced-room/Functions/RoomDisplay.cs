...
using PepperDash.Essentials.Devices.Common.Shades;
...
namespace essentials_advanced_room.Functions
{
    public class RoomDisplay: IHasDefaultDisplay, ILogClassDetails
    {
        ...
        private List<IRoutingSink> Displays;

        public IRoutingSink DefaultDisplay { get; private set; } // IHasDefaultDisplay
        ...
        {
            Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayLifter displayProps {1}", ClassName, displayProps.Lifter == null ? "== null" : displayProps.Lifter.DeviceKey);
            if (displayProps.Lifter != null)
            {
                ...
                //Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayLifter DefaultLifter {1}", ClassName, DefaultLifter == null ? "== null" : device_.Key);
                if (device_ is ShadeBase)
                {
                    //Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayLifter device_ is ShadeBase", ClassName);
                    var config_ = ConfigReader.ConfigObject.Devices.Find(x => x.Key == device_.Key);
                    //Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayLifter config_ {1}", ClassName, config_ == null ? "== null" : config_.GetType().ToString());
                    var props_ = JsonConvert.DeserializeObject<RelayControlledShadeConfigProperties>(config_.Properties.ToString());
                    //Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayLifter props_ {1}", ClassName, props_ == null ? "== null" : props_.GetType().ToString());
                    if(props_?.Relays != null)
                    {
                        ...
                        Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayLifter OPEN relay_ {1}", ClassName, relay_ == null ? "== null" : relayConfig_.PortNumber.ToString());
                        ...
                        Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayLifter CLOSE relay_ {1}", ClassName, relay_ == null ? "== null" : relayConfig_.PortNumber.ToString());
                        relay_.OutputIsOnFeedback.OutputChange += Lifter_CloseRelay_OutputIsOnFeedback_OutputChange;
                    }
                }
                //Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayLifter device_ configured", ClassName);
            }
        }
        private void SetupDefaultDisplayScreen(DisplayPropsConfig displayProps)
        {
            Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayScreen displayProps {1}", ClassName, displayProps.Screen == null ? "== null" : displayProps.Screen.DeviceKey);
            if (displayProps.Screen != null)
            {
                ...
                Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayScreen DefaultScreen {1}, {2}", ClassName, DefaultScreen == null ? "== null" : device_.Key, device_.GetType().Name.ToString());

                if (device_ is ShadeBase)
                {
                    //Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayScreen device_ is ShadeBase", ClassName);
                    var config_ = ConfigReader.ConfigObject.Devices.Find(x => x.Key == device_.Key);
                    //Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayScreen config_ {1}", ClassName, config_ == null ? "== null" : config_.GetType().Name.ToString());
                    try // this is just for debugging, it is only for screens that use native relays 
                    {
                        var props_ = JsonConvert.DeserializeObject<RelayControlledShadeConfigProperties>(config_.Properties.ToString());
                        if (props_ != null)
                        {
                            Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayScreen props_ {1}", ClassName, props_ == null ? "== null" : props_.GetType().Name.ToString());
                            if(props_.Relays != null)
                            {
                                IOPortConfig relayConfig_ = props_.Relays.Open;
                                IKeyed csKey_ = DeviceManager.GetDeviceForKey(relayConfig_.PortDeviceKey);
                                var relay_ = (csKey_ as ISwitchedOutputCollection).SwitchedOutputs[relayConfig_.PortNumber];
                                Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayScreen OPEN relay_ {1}", ClassName, relay_ == null ? "== null" : relayConfig_.PortNumber.ToString());
                                relay_.OutputIsOnFeedback.OutputChange += Screen_OpenRelay_OutputIsOnFeedback_OutputChange;

                                relayConfig_ = props_.Relays.Close;
                                csKey_ = DeviceManager.GetDeviceForKey(relayConfig_.PortDeviceKey);
                                relay_ = (csKey_ as ISwitchedOutputCollection).SwitchedOutputs[relayConfig_.PortNumber];
                                Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayScreen CLOSE relay_ {1}", ClassName, relay_ == null ? "== null" : relayConfig_.PortNumber.ToString());
                    ...
                    {
                        Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayScreen - doesn't contain native relays: {1}", ClassName, e.Message);
                    }

                }
                //Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplayScreen device_ configured", ClassName);
            }
        }
        private void SetupDefaultDisplay()
        {
            ...
            {
                DefaultDisplay = DeviceManager.GetDeviceForKey(config.DefaultDisplayKey) as IRoutingSink;
                if(!Displays.Contains(DefaultDisplay))
                    Displays.Add(DefaultDisplay);
                Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplay {1}", ClassName, DefaultDisplay == null ? "== null" : DefaultDisplay.Key);
                if (DefaultDisplay != null)
                {
                    var dispTwoWay = DefaultDisplay as IHasPowerControlWithFeedback;
                    //Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplay, IHasPowerControlWithFeedback {1}", ClassName, dispTwoWay == null ? "== null" : DefaultDisplay.Key);
                    ...
                    //Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplay, IWarmingCooling {1}", ClassName, dispWarmCool == null ? "== null" : DefaultDisplay.Key);
                    ...
                    if (dispConfig_ != null)
                    {
                        //Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplay, found key in config", ClassName);
                        defaultDisplayConfig = JsonConvert.DeserializeObject<DisplayPropsConfig>(dispConfig_.Properties.ToString());
                        //Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplay props_ {1}", ClassName, props_ == null ? "== null" : config.DefaultDisplayKey);
                     ...
                     Debug.LogMessage(LogLevel, "{0} SetupDefaultDisplay config.DefaultDisplayKey {1}", ClassName, config.DefaultDisplayKey == null ? "== null" : config.DefaultDisplayKey);
           }
        }

        private void Screen_OpenRelay_OutputIsOnFeedback_OutputChange(object sender, FeedbackEventArgs e)
        {
            Debug.LogMessage(LogLevel, "{0} Screen_OpenRelayFeedback: {1}", ClassName, e.BoolValue);
            if(e.BoolValue)
            {
        ...
        {
            Debug.LogMessage(LogLevel, "{0} Screen_CloseRelayFeedback: {1}", ClassName, e.BoolValue);
            if (e.BoolValue)
            {
                if (defaultDisplayConfig != null && DefaultDisplay != null)
                {
        ...
        {
            Debug.LogMessage(LogLevel, "{0} Lifter_OpenRelayFeedback: {1}", ClassName, e.BoolValue);
            if (e.BoolValue)
            {
                if (defaultDisplayConfig != null && DefaultDisplay != null)
                {
        ...
        {
            Debug.LogMessage(LogLevel, "{0} Lifter_CloseRelayFeedback: {1}", ClassName, e.BoolValue);
            if (e.BoolValue)
            {
                if (defaultDisplayConfig != null && DefaultDisplay != null)
                {
        ...
        private void SetupDestinationList()
        {
            if (!String.IsNullOrEmpty(config.DestinationListKey))
            {
                ...
                Debug.LogMessage(LogLevel, "{0} SetupDestinationList {1}", ClassName, destinationList == null ? "== null" : destinationList.Count.ToString());
                Displays.Clear();
                if (destinationList != null)
                {
                    foreach (var destination in destinationList)
                    {
                        ...
                        Debug.LogMessage(LogLevel, "{0} SetupDisplays, IHasPowerControlWithFeedback {1}", ClassName, dispTwoWay == null ? "== null" : dest.Key);
                        if (dispTwoWay != null)
                        {
                            dispTwoWay.PowerIsOnFeedback.OutputChange -= PowerIsOnFeedback_OutputChange;
                            dispTwoWay.PowerIsOnFeedback.OutputChange += PowerIsOnFeedback_OutputChange;
                        ...
                        Debug.LogMessage(LogLevel, "{0} SetupDisplays, IWarmingCooling {1}", ClassName, dispTwoWay == null ? "== null" : dest.Key);
                        if (dispWarmCool != null)
                        {
                            dispWarmCool.IsWarmingUpFeedback.OutputChange -= IsWarmingUpFeedback_OutputChange;
                            dispWarmCool.IsWarmingUpFeedback.OutputChange += IsWarmingUpFeedback_OutputChange;
        ...
        void IsCoolingDownFeedback_OutputChange(object sender, FeedbackEventArgs e)
        {
            ...
            Debug.LogMessage(LogLevel, msg_);
        ...
        {
            //Debug.LogMessage(LogLevel, "{0} SetPowerOn, Displays {1}", ClassName, Displays==null?"== null": Displays.Count.ToString());
            foreach (var display in Displays)
            {
                var display_ = display as IHasPowerControl;
                //Debug.LogMessage(LogLevel, "{0} Display {1}", ClassName, display_ == null ? display.GetType().Name : display.Key);
                if (display_ != null)
                {
                    Debug.LogMessage(LogLevel, "{0}[{1}] SetPowerOn", ClassName, display.Key);                        
                    display_.PowerOn();
                }
            }
        }
        ...
        {
            //Debug.LogMessage(LogLevel, "{0} SetPowerOff, Displays {1}", ClassName, Displays == null ? "== null" : Displays.Count.ToString());
            foreach (var display in Displays)
            {
                var display_ = display as IHasPowerControl;
                if (display_ != null)
                {
                    Debug.LogMessage(LogLevel, "{0}[{1}] SetPowerOff", ClassName, display.Key);
                    display_.PowerOff();
                }
            }
        }
