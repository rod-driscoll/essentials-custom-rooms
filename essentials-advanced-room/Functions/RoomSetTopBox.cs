...
namespace essentials_advanced_room.Functions
{
    ...
    public class RoomSetTopBox: IHasDefaultSetTopBox, ILogClassDetails
    {
        ...
        public RoomSetTopBox(Config config)
        {
            ...
            Debug.LogMessage(LogLevel, "{0} constructor starting", ClassName);
            this.config = config;
            IKeyed device_;
            try
            {
                ...
                {
                    Debug.LogMessage(LogLevel, "{0} DefaultSetTopBoxKey: {1}", ClassName, config.DefaultSetTopBoxKey);
                    //Debug.LogMessage(LogLevel, "{0} SetTopBoxPresetsURL: {1}", ClassName, config.SetTopBoxPresetsURL == null ? "== null" : config.SetTopBoxPresetsURL);
                    device_ = DeviceManager.GetDeviceForKey(config.DefaultSetTopBoxKey);
                    if (device_ != null)
                    {
                        Debug.LogMessage(LogLevel, "{0} DefaultSetTopBox is {1}", ClassName, device_.GetType().Name);
                        
                        // this doesn't work if the interface is not from the same assembly
                        DefaultSetTopBox = device_ as ISetTopBoxControls; 
                        Debug.LogMessage(LogLevel, "{0} ISetTopBoxControls {1}", ClassName, DefaultSetTopBox == null?"== null":"exists");
                        
                        var stb_ = device_ as IRSetTopBoxBase; // null for non crestron ir ports
                        Debug.LogMessage(LogLevel, "{0} IRSetTopBoxBase, {1} a Crestron native IR port", ClassName, stb_ == null?"Not":"Is");
                        var stbPortController_ = device_ as IHasIrOutputPortController; // null, not the same assembly
                        Debug.LogMessage(LogLevel, "{0} IHasIrOutputPortController {1}", ClassName, stbPortController_ == null ? "== null" : "exists");
                        ...
                        else if (stbPortController_ != null) // non-Crestron IR port, won't run, not part of assembly
                        {
                            try
                            {
                                Debug.LogMessage(LogLevel, "{0} stbPortController.IrPort {1}", ClassName, stbPortController_ == null ? "== null" : "exists");
                                if (stbPortController_.IrPort != null)
                                    stbPortController_.IrPort.DriverLoaded.OutputChange += DriverLoaded_OutputChange;
                            }
                            catch (Exception e)
                            {
                                Debug.LogMessage(LogLevel, "{0} IHasIrOutputPortController ERROR: {1}", ClassName, e.Message);
                            }
                        }
                        ...
                        if (stbPresets_.TvPresets != null && config.SetTopBoxPresetsURL != null)
                        {
                            ...
                            Debug.LogMessage(LogLevel, "{0} ISetTopBoxControls, setting path: '{1}'", ClassName, config.SetTopBoxPresetsURL);
                        }
                    }
                }
            }
            ...
            {
                Debug.LogMessage(LogLevel, "{0} RoomSetTopBox constructor ERROR: {1}", ClassName, e.Message);
            }
        }
                            
        private void DriverLoaded_OutputChange(object sender, FeedbackEventArgs e)
        {
            Debug.LogMessage(LogLevel, "{0} DriverLoaded, DefaultSetTopBox {1}", ClassName, DefaultSetTopBox==null?"== null": DefaultSetTopBox.GetType().Name);
            if (DefaultSetTopBox != null)
            {
                ...
                if (stb_ != null)
                {
                    if (stb_.IrPort != null)
                    {
                        ...
                        //Debug.LogMessage(LogLevel, "{0} IrPort {1}", ClassName, irPort_ == null ? "== null" : "exists");
                        if (irPort_ != null)
                        {
                            Debug.LogMessage(LogLevel, "{0} DefaultSetTopBox IrPort: '{1}', path: '{2}', loaded: {3}", ClassName, irPort_.Name, irPort_.DriverFilepath, irPort_.DriverIsLoaded);
                            //irPort_.PrintAvailableCommands();
                            //foreach (var command in irPort_.IrFileCommands)
                            //    Debug.LogMessage(LogLevel, "{0} command: {1}", ClassName, command);
                        }
                    }
                    Debug.LogMessage(LogLevel, "{0} TvPresets: '{1}'", ClassName, stb_.TvPresets == null ? "== null" : stb_.TvPresets.Name);
                    Debug.LogMessage(LogLevel, "{0} stb_.TvPresets.ImagesLocalHostPrefix: {1}", ClassName, stb_.TvPresets.ImagesLocalHostPrefix == null ? "== null" : stb_.TvPresets.ImagesLocalHostPrefix);
        ...
        private void TryToLoadFromAssembly()
        {
            ...
            Debug.LogMessage(LogLevel, "{0} CheckIfAssemblyLoaded({1}): {2}", ClassName, assemblyName_, isloaded_);

            // exists below
            var loadedAssembly_ = PluginLoader.LoadedAssemblies.FirstOrDefault(a => a.Name == assemblyName_);
            Debug.LogMessage(LogLevel, "{0} LoadedAssembly {1}", ClassName, loadedAssembly_ == null ? "== null" : "exists");

            //exists below
            var assembly_ = loadedAssembly_.Assembly;
            Debug.LogMessage(LogLevel, "{0} Assembly {1}", ClassName, assembly_ == null ? "== null" : "exists");
            ...
            try
            {
                Type[] ctypes_ = assembly_.GetTypes();
                ...
                //Debug.LogMessage(LogLevel, "{0} typeName: {1}, ", ClassName, typeName_);
                var type_ = ctypes_.FirstOrDefault(x => x.Name == typeName_);
                Debug.LogMessage(LogLevel, "{0} type: {1} ", ClassName, type_ == null ? "== null" : "exists");
                if (type_ != null)
                {
                    //Debug.LogMessage(LogLevel, "{0} interfaceName_: {1}, ", ClassName, interfaceName_);
                    var interfaces_ = type_.GetInterfaces();
                    //Debug.LogMessage(LogLevel, "{0} interfaces_ {1}, ", ClassName, interfaces_ == null ? "==null" : "exist");
                    if (interfaces_ != null)
                    {
                        var interface_ = interfaces_.FirstOrDefault(x => x.Name == interfaceName_);
                        Debug.LogMessage(LogLevel, "{0} interface: {1} {2}", ClassName, interfaceName_, interface_ == null ? "== null" : "exists");
                        if (interface_ != null)
                        {
                    ...
                    //Debug.LogMessage(LogLevel, "{0} propertyName_: {1}, ", ClassName, propertyName_);
                    var property_ = type_.GetProperty(propertyName_);
                    Debug.LogMessage(LogLevel, "{0} property: {1} {2} ", ClassName, propertyName_, property_ == null ? "== null" : "exists");
                    if (property_ != null)
                    {
                        //trying to do this...
                    ...
                    Debug.LogMessage(LogLevel, "{0} method: {1} {2} ", ClassName, methodName_, method_ == null ? "== null" : "exists");
                    if (method_ != null)
                    {
            ...
            {
                Debug.LogMessage(LogLevel, "{0} ERROR: {1}", ClassName, e.Message);
            }
            /* // null below
            var interfaceType_ = assembly_.GetType(interfaceName_);
            Debug.LogMessage(LogLevel, "{0} interfaceType_ {1}", ClassName, interfaceType_ == null ? "== null" : "exists");
            ...
            if(interfaceType_ != null)
            {
                ...
                Debug.LogMessage(LogLevel, "{0} subscribed to DriverLoaded_OutputChange", ClassName);
            }
            // errors
            var interfaces_ = interfaceType_.GetInterfaces();
            Debug.LogMessage(LogLevel, "{0} Interfaces {1}", ClassName, interfaces_ == null ? "== null" : "exists");
            if(interfaces_ != null) {}
            // errors
            var interface_ = interfaceType_.GetInterfaces().FirstOrDefault(a => a.Name == interfaceName_);
            Debug.LogMessage(LogLevel, "{0} Interface {1}", ClassName, interface_ == null ? "== null" : "exists");
            */
